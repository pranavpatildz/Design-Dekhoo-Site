...
const getFurniture = async (req, res) => {
  ...
  try {
    // Temporarily removing city filtering to ensure seeded data (without shopOwnerId) is fetched.
    // The public route will now only filter by category, material, minPrice, maxPrice.
...
const getShopOwnerFurniture = async (req, res) => {
  try {
    const furniture = await Furniture.find({ shopOwnerId: req.shopOwner.id }); // Removed .sort({ createdAt: -1 })
    res.json(furniture);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
